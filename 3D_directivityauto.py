...
output_directory = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'postAcoustics/figures')
...
for x0 in x0_values:
    ...
    for idx_theta, theta in enumerate(theta_values):
        ...
        for idx_phi, phi in enumerate(phi_values):
            ...
            # Calculate distance r_theta_phi from centroid to the new observer position x0_theta_phi
...
# Save the figure in the 'postAcoustics' folder
output_filename = os.path.join(output_directory, f'SPL_Directivity_3DViridis_{x0}_{script_directory}.pdf')
plt.savefig(output_filename, bbox_inches='tight', dpi=300)
print(f'SPL Directivity 3D plot saved to {output_filename}')

# Display the plot
plt.show()

# Repeat the process for the 'jet' colormap
fig = plt.figure(figsize=(10, 8))
ax = fig.add_subplot(1, 1, 1, projection='3d')
ax.grid(False)
ax.axis('off')
...
# Save the figure in the 'postAcoustics' folder
output_filename = os.path.join(output_directory, f'SPL_Directivity_3DJet_{x0}_{script_directory}.pdf')
plt.savefig(output_filename, bbox_inches='tight', dpi=300)
print(f'SPL Directivity 3D plot saved to {output_filename}')
