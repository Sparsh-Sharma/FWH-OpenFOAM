...
num_angles = 181  # Number of angles to consider (0 to 360 degrees)
...
x0_values = [0.2, 0.4, 0.6]  # Add the x0 values you want to consider
...
for x0 in x0_values:
    ...
    for idx, theta in enumerate(theta_values):
        ...
        # Calculate distance r from centroid to the new observer position x0_theta
        ...
        SPL_values[idx, :] = 10 * np.log10(val_NC_theta[indices_of_interest] + 1e-10)  # Add a small value to avoid zero
    ...
    for i, freq in enumerate(freq_of_interest):
        ...
        ax.plot(np.radians(theta_values), SPL_values[:, i], label=f'Directivity at {freq} Hz', color='b')
        ax.set_rlabel_position(90)
        ax.set_theta_zero_location('N')
        ax.set_rmax(np.max(SPL_values) + 5)  # Set maximum radius for better visibility

        ax.set_title(f'Sound Pressure Level Directivity at {freq} Hz (x0={x0})', va='bottom')
        ax.grid(True)

        plt.savefig(f'Directivity_Polar_plot_x0_{x0}_freq_{freq}Hz.pdf')  # Save the polar plot as an image file
